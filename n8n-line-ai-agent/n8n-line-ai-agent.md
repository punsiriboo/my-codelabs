...
4. ที่ช่อง **Model** ให้เลือกโมเดลเป็น `models/gemini-2.5-flash` หรือ `models/gemini-2.5-flash-lite`
...
4.1. **Rename node:** ตั้งชื่อ node เป็น `Send Flex Message` (เพื่อให้เข้าใจง่ายว่า node นี้ทำหน้าที่อะไร)
4.2. **Alt Text:** ใส่ข้อความที่จะแสดงใน Notification เมื่อมีข้อความเข้า เช่น `เมนูอาหารแนะนำ`
4.3. **Flex Message JSON:** นำโค้ด JSON ที่เตรียมไว้ด้านล่างมาวางลงในช่องนี้
