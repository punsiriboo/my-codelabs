...
4. ที่ช่อง **Model** ให้เลือกโมเดลเป็น `models/gemini-2.5-flash-lite` หรือ `models/gemini-2.5-flash-lite`
...
4. ส่งคำตอบที่ได้จาก AI Agent กลับไปยังผู้ใช้
5. Save และ Publish workflow และทำการทดสอบ
---
...
3.2. ตั้งค่า **Context Window Length** (จำนวนข้อความที่ต้องการจำ) ตามต้องการ ในตัวอย่างคือ 5
...
จาก node Reply ที่มีอยู่แล้วเราจะเข้าไปทำการแก้ไขข้อคามตอบกลับ
...
5. แก้ไข System prompt ให้เรียกใช้งาน Tools
...
กลับมาที่ Workflow ใน n8n ของเรา เพื่อเชื่อมต่อเครื่องมือนี้เข้ากับ AI Agent ให้สามารถเรียกเครื่องมือเพื่อส่ง Flex Message ได้ 

3.1. ไปที่โหนด **AI Agent**
3.2. คลิกที่เครื่องหมาย **`+`** ในส่วนของการเชื่อมต่อ **Tools** (Memory/Tool)
3.3. ค้นหาคำว่า `LINE` และเลือก **"Line Messaging Tool"** (สังเกตคำอธิบาย: *Interact with the Line Messaging API*)

![Adding LINE Tool](images/9.2.png)

### 3.4. ตั้งค่า Tool

เมื่อได้ LINE Messaging Tool มาแล้ว ให้ทำการตั้งค่าดังนี้:

- ***Operation:** เลือก **Send**
- **User ID:** ใส่ Expression เพื่อดึง User ID จาก LINE:
   - `{{ $("Line Messaging Trigger").item.json.source.userId }}`
- **Messages:** กดปุ่ม **"Add Message"** แล้วเลือก **type: Flex Message**

![Adding LINE Tool](images/9.3.png)

### 4. ตั้งค่า Flex Message JSON

3.5.1. **Rename node:** ตั้งชื่อ node เป็น `Send Flex Message` (เพื่อให้เข้าใจง่ายว่า node นี้ทำหน้าที่อะไร)
3.5.2. **Alt Text:** ใส่ข้อความที่จะแสดงใน Notification เมื่อมีข้อความเข้า เช่น `เมนูอาหารแนะนำ`
3.5.3. **Flex Message JSON:** นำโค้ด JSON ที่เตรียมไว้ด้านล่างมาวางลงในช่องนี้

**Flex Message JSON: ตัวอย่างโค้ด JSON สำหรับเมนูอาหาร:**
```json
...
เพื่อให้ AI Agent รู้ว่าควรเรียกใช้ Tool (Send Flex Message) เมื่อไหร่ เราต้องเพิ่มคำสั่งใน System prompt

5.1. ไปที่ node **Google Gemini Chat Model** (หรือ Model ที่คุณเลือกใช้)
5.2. ในส่วนของ **System Instruction**  ให้เพิ่มคำสั่งเกี่ยวกับการใช้งาน Tool:

```
Trigger: ลูกค้าต้องการดูเมนู เช่น ขอเมนู, มีเมนูอะไรแนะนำ 
- Action: Tool: Send Flex Message
```


<aside class="positive">
<strong>Note:</strong> การเพิ่มคำสั่งนี้จะทำให้ AI รู้ว่าควรเรียกใช้ Tool เมื่อผู้ใช้ถามเกี่ยวกับเมนูอาหาร ซึ่งจะทำให้สามารถส่ง Flex Message ที่สวยงามได้
</aside>

![Completed Workflow](images/9.5.png)

### 6. ทดสอบผลลัพธ์ใน LINE
...
### ขั้นตอนทั้งหมด
1. แก้ไข System Instruction โดยใข้ ตัวอย่าง Prompt สำหรับร้านอาหาร
...
ซึ่งจากผลลัพธ์จะเห็นว่าคุณสามารถสร้าง AI Chatbot บน LINE ที่สามารถตอบคำถามได้อย่างเป็นธรรมชาติ และส่ง Flex Message ที่สวยงามได้แล้ว
